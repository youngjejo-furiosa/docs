...
description: "To run your own trained, fine-tuned, or quantized model, you must first convert it into a model artifact using Furiosa-LLM."
---
...
<Note>
  ...
  **legacy and scheduled for deprecation**. New work should use the
  [Furiosa Executable Bundle (FXB)](/furiosa-llm/fxb) format and the `fxb` command instead,
...
Starting with 2026.2, `ArtifactBuilder` auto-applies a **model-specific bucket preset**
when no explicit `BucketConfig` is provided. The preset expands to a set of optimized
prefill, decode, and append buckets covering the model's full maximum context (or a
smaller `ModelConfig.max_model_len`, if set). For most users this removes the need to
specify buckets by hand.

You can still customize the build: supply an explicit `BucketConfig` to override the
preset, or use `ModelConfig.max_model_len` to cap the context length of the generated
buckets. You can also specify parallelization strategies such as tensor parallelism and
pipeline parallelism via `ParallelConfig` to efficiently utilize multiple NPUs for
inference.

The simplest path is to rely on the preset and only provide the parallelization
strategy:
...
builder = ArtifactBuilder(
    ...
    model_config=ModelConfig(
        max_model_len=4096,  # Optional: cap the context length of the generated buckets
    ),
...
If you need full control over the bucket shapes, supply a `BucketConfig` explicitly.
When any bucket field is populated, the preset is skipped and the explicit configuration
is used as-is:
